...
function renderMenu() {
  ...
  if (drinkGroup) drinkGroup.innerHTML = byCat('drink').map(itemButton).join('');
}
...
function getDrinkTotalCharge(drink) {
  return Number(drink.upsizeCharge || 0);
}

function getDrinkChargeLabel(drink) {
  const comboCharge = getDrinkTotalCharge(drink);
...
function openDrinkChoiceModal(comboItem) {
  ...
  if (list) {
    ...
    list.innerHTML = drinks.length ? drinks.map(drink => `
      <button class="drink-choice-btn" style="background:#ffffff!important;color:#1a1009!important;-webkit-text-fill-color:#1a1009!important;border:3px solid #ffb000!important;border-radius:14px!important;min-height:64px!important;padding:16px 22px!important;opacity:1!important;filter:none!important;" onclick="confirmComboDrink('${drink.id}')">
        <span class="drink-name" style="color:#1a1009!important;-webkit-text-fill-color:#1a1009!important;opacity:1!important;font-weight:900!important;">${escapeHtml(drink.name)}</span>
        <small class="drink-price" style="color:#b45309!important;-webkit-text-fill-color:#b45309!important;opacity:1!important;font-weight:900!important;">${getDrinkChargeLabel(drink)}</small>
      </button>`).join('') : '<p class="muted">No drink items have been added yet. Please ask an admin to add drinks from Admin → Add Drinks.</p>';
  }
...
function confirmComboDrink(drinkId) {
  ...
  addMenuItemToCart(pendingComboItem, drink.name, getDrinkTotalCharge(drink));
...
function openAdminPopout(panelId) {
  ...
  if (panelId === 'drinkAdminPanel') renderAdminDrinks();
...
function saveAdminDrink() {
  ...
  const extraEl = document.getElementById('drinkExtraCharge');
  const costEl = document.getElementById('drinkCost');
  const name = nameEl ? nameEl.value.trim() : '';
  const price = Number(priceEl ? priceEl.value || 0 : 0);
  const upsizeCharge = Number(extraEl ? extraEl.value || 0 : 0);
  const cost = Number(costEl ? costEl.value || 0 : 0);

  if (!name || price < 0 || upsizeCharge < 0 || cost < 0 || Number.isNaN(price) || Number.isNaN(upsizeCharge) || Number.isNaN(cost)) {
    return alert('Please enter a valid drink name, drink price, extra/upsize charge, and item cost. Only the extra/upsize charge is added to burger combos.');
  }

  const data = getData();
  const existing = data.menu.find(item => item.category === 'drink' && item.name.toLowerCase() === name.toLowerCase());

  if (existing) {
    existing.price = price;
    existing.cost = cost;
    existing.upsizeCharge = upsizeCharge;
    existing.recipe = existing.recipe || [];
  } else {
    data.menu.push({ id: uid(), name, price, cost, upsizeCharge, category: 'drink', recipe: [] });
  }
...
function editAdminDrink(id) {
  ...
  const extraEl = document.getElementById('drinkExtraCharge');
  if (extraEl) extraEl.value = drink.upsizeCharge || 0;
  document.getElementById('drinkCost').value = drink.cost || 0;
...
function renderAdminDrinks() {
  ...
  el.innerHTML = drinks.length ? drinks.map(drink => `
    <div class="list-row">
      <div>
        <strong>${escapeHtml(drink.name)}</strong><br>
        <small>Ala Carte Drink Price: ${peso.format(Number(drink.price || 0))} · Combo Extra/Upsize Charge: ${peso.format(Number(drink.upsizeCharge || 0))} · Item Cost: ${peso.format(Number(drink.cost || 0))}</small>
      </div>
      <div class="row-actions">
